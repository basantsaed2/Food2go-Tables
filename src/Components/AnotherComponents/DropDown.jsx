       ...
       return (
              <div className="w-full mx-auto relative h-12 " ref={ref}>
                     <button
                            ...
                            className={`flex ${iconDirection ? 'flex-row-reverse' : 'flex-row'} items-center justify-between w-full py-2 px-3 h-14 shadow ${border ? "border-2 border-gray-400" : ''} rounded-xl outline-none bg-secoundBgColor text-2xl text-fourthColor`}
                            onClick={handleOpen}
                     >
                            <div className="text-mainColor text-2xl">{iconMenu}</div>
                            <span className="eleValueDropDown">{stateoption}</span>
                            <IoIosArrowDown className={`${openMenu ? "rotate-180" : "rotate-0"} text-fourthColor text-2xl transition-transform duration-300`} />
                     </button>
